...
## 第四個問題：HOST 有沒有幫飛機配平?（`--neutral`)

這是目前**最大的未驗證風險**,而且它跟上面三個不一樣 —— 上面三個問錯了要調參數,
這個問錯了**整個訓練環境是假的**。

我們所有的結論都建立在一句話上:**搖桿置中,49 秒撞地。** 那是我們的環境量的。
如果真的 HOST 開局就把飛機配平好,那:

- 防墜地板存在的理由消失一半
- 「什麼都不做 = 65% 勝率」這條門檻整個作廢
- v4 練的是一個不存在的世界

**平常的探針答不了這個問題。** 它飛的是 `LevelPolicy` —— 一個維持高度的自動駕駛。
它會把要測的漂移**修掉**,所以「會配平的 HOST」和「不會配平的 HOST」在它眼裡
長得一模一樣。
...
```bash
.venv/Scripts/python.exe backend/competition/probe.py --neutral --seconds 120
```

然後照下面的步驟 2、3 開 HOST、按 INIT、按 START。**60 秒就夠**,不用等滿。

`--neutral` 送的是純中立指令:副翼 0、升降舵 0、方向舵 0、油門 0.8
(OBS 封包沒有油門可讀,0.8 是參考環境的開局值)。

### 報告會直接給答案

```
trim_verdict: host does NOT trim: -19,340 ft over 49 s (-395 ft/s).
              Same as ours, so the floor and every baseline measured
              against it stand
```
...
| 判定 | 意思 | 要做什麼 |
|---|---|---|
| `does NOT trim` | 跟我們一樣會俯衝 | **什麼都不用改**,所有結論成立 |
...
### 已回答(2026-09-26):**不是飛機的問題,是起始姿態**

```
verdict: our plant descends +14% against the host's (305 vs 267 ft/s)
```

但 `+14%` 這個標題是誤導的。看每十秒**各自掉多少**:

| 區間 | HOST | 我們 |
|---|---|---|
| **0–10 秒** | **129 ft** | **1,007 ft** |
| 10–20 | 1,795 | 2,869 |
| 20–30 | 3,269 | 3,741 |
| 30–40 | 3,709 | 3,915 |
| 40–50 | 3,566 | 3,704 |

**後段兩邊差 5~10%,整個落差是在頭 20 秒造出來的,之後就不再擴大。**
速度差也在收斂:t=10 差 61 節,t=50 只差 28 節。

**兩架不同的飛機會一路都不一樣。只在開場不一樣,那是起始姿態。**

我們的初始條件寫死 `ic/theta-deg = 0`。但 18,084 ft、340 KCAS 的**水平飛行
需要正攻角** —— 俯仰角 0 等於一開場就升力不足,立刻往下掉。掃一遍起始俯仰角:

| 起始俯仰角 | 對 HOST 軌跡的最大誤差 |
|---|---|
| **0°(現在的)** | **2,800 ft** |
| 3° | 1,403 ft |
| 5° | 563 ft |
...
| **6.5°** | **185 ft** |
| 7° | 407 ft |
| JSBSim `do_simple_trim` | 15,187 ft(會爬升) |

6.5° 把 50 秒的誤差從 2,800 ft 降到 185 ft —— 掉了 12,468 呎,誤差 **1.5%**。

`do_simple_trim` 跑掉是另一個證據:它連**升降舵**也配平,飛機就爬升了。
HOST 不是那樣 —— **它配平了姿態,沒配平升降舵**,所以會撐一下再掉。

### 然後量了,擬合是錯的

```
"host_opening": { "pitch_deg": 0.0, "alpha_deg": 0.02, "ours_pitch_deg": 0.0 }
```

**HOST 的起始俯仰角是 0,跟我們一模一樣。** 上面那張 6.5° 的表是**過度擬合** ——
它找到一個能把高度曲線對上的數字,但那個數字描述的機制不存在。照著改,曲線會對,
物理是假的。**那比對不上更糟**,所以環境沒有動。

> 這就是為什麼 `host_opening` 印的是從檔案裡**讀出來**的值,不是解出來的值。

### 目前所在:是什麼讓 HOST 的機頭抬著?
...
**我們的攻角全程貼在 0 附近**(−0.6、−0.4、−0.5)。飛機沒有在產生升力,只是把
機頭指下去跟著掉 —— 那是「升降舵中立 + 配平中立」的平衡點。HOST 撐了十秒只掉
129 呎,它一定在產生正攻角。
...
**兩架飛機的氣動狀態逐點相同,差 0.1 度。** 俯仰角也都收斂到 −19°。不是配平、
不是重心、不是俯仰力矩 —— 那些都會讓攻角分岔。

唯一的差別是過渡快慢。把我們的軌跡往後平移:

| 平移 | HOST t=10(−5.8) | t=20(−16.8) | 最大俯仰誤差 | 最大高度誤差 |
|---|---|---|---|---|
| 0 秒 | −13.7 | −19.3 | 7.9° | 2,813 ft |
| **6.2 秒** | **−5.8** | **−16.8** | **1.7°** | **555 ft** |

**6.2 秒不是擬合出來的。** 它是 `374 ÷ 60` —— 報告裡 `frames_with_position_held`
那個數字。**HOST 在 INIT 和 START 之間把飛機凍住,同時照樣送封包**,而我的重播
從第 0 幀就開始飛,等於拿 t 去比 t+6.2。

`_from_first_motion` 現在會把那段跳掉,從位置真正開始變化的那一幀對齊,
並且印出跳了幾幀:

```
"held_frames_skipped": 374,
"held_seconds_skipped": 6.23,
```

**那個數字同時也是這個解釋的驗證。** 印 374 就成立;印 0 就代表凍結那段
不在回合裡,平移和 374 的吻合只是巧合,要重找。
...
"held_frames_skipped": 374,
"held_seconds_skipped": 6.23,
"nose_gap_at_10s_deg": 0.1,
"verdict": "our plant matches the host's to 4% on descent rate"
```

| t | HOST | 我們 | 差 | HOST 俯仰 | 我們俯仰 |
|---|---|---|---|---|---|
| 10 秒 | 17,074 ft | 17,077 ft | **+3 ft** | −13.8 | −13.7 |
| 20 秒 | 14,217 ft | 14,208 ft | **−8 ft** | −19.0 | −19.3 |
| 30 秒 | 10,587 ft | 10,467 ft | −120 ft | −19.4 | −20.2 |
| 50 秒 | 3,596 ft | 2,848 ft | −748 ft | −15.8 | −17.6 |

**一萬八千呎裡差三呎。** 機頭落差從 −7.9° 變成 **0.1°**。

**環境是對的,不用改任何東西。**

殘留的誤差在後段:50 秒時差 748 ft(掉了 14,488 ft 的 5%),速度差 3%。
兩邊那時都已經接近 Mach 1,跨音速阻力的一點點差異會累積。**不追**:
一回合 300 秒,飛機不會待在彈道俯衝裡,而 10–20 秒那段(差 3~8 呎)才是
纏鬥真正發生的區間。
...
```bash
scripts/evaluate.bat --baseline --ground-avoidance models/competition/run1
```

`--baseline` 多跑一列**搖桿完全置中**的「什麼都不做」,同樣的種子、同樣的對手、
同樣的地板。**這是每一個訓練出來的模型必須跨過的門檻,而且它會移動** —— 地板改
好之後基準線自己就從 65%/+55 變成 65%/+51,你改的東西也在幫基準線變強。

不給 session、只給 `--baseline`,就單獨量基準線:
...
沒有這一欄就分不出兩種 0% 墜毀:一種是策略自己會飛,一種是地板抱著它飛完全程。
實測 `do nothing` 配地板是 **6%** —— 地板幾乎沒動,那 65% 的勝率和 +51 的分差
是搖桿置中自己拿到的。
...
防墜地板(`safety.py`)是**外掛在策略外面的一層規則**,不在權重裡面 ——
所以它可以現裝現拆,不用重練。這讓一個問題變得很便宜:

> 這個模型如果不摔死,它會贏嗎?

```bash
scripts/evaluate.bat --ground-avoidance models/competition/run1
```

`--ground-avoidance` 會給**每一個** session 裝上地板,包含沒有用地板練過的。
反過來:

```bash
scripts/evaluate.bat --no-ground-avoidance models/competition/v2
```

把地板拆掉,看策略自己學到了什麼。

**借來的地板會寫在標籤上。** 報表印的是 `run1 +floor` 或 `v2 -floor`,不是
`run1`、`v2`。原因很簡單:這個數字**不是那個模型自己飛出來的**,不標清楚就是
在騙自己。沒有借用的時候不加後綴。

## 為什麼要有這個開關

第一次量 run1(練滿 5,000,000 步)得到的結果是:

| | won | crashed | margin | 152-500m |
|---|---|---|---|---|
| 什麼都不做 | 0% | 100% | +18 | 0.2 |
| 什麼都不做 + 地板 | **65%** | **0%** | +55 | 0.2 |
| run1 | 0% | 95% | **+63** | 0.3 |

run1 的**分差是三者最高的** —— 它確實學會了累積位置優勢。然後它把自己摔死,
而依主辦方表 3,墜毀是無視分數直接判輸。

**五百萬步,輸給四行規則。**

這不是「模型不夠好」,是**折扣視野**的問題:γ=0.99、1x action repeat、60 Hz,
1/(1−γ) = 100 個決策 = **1.7 秒**。撞地的代價在 1.7 秒之外,策略看不到它。
所以 v2 改的是 γ=0.995 + action repeat 6(視野 20 秒)、margin 獎勵、以及
訓練時就掛上地板。

有了 `--ground-avoidance`,「run1 少的到底是不是只有一塊地板」這個問題
不用重練就能回答。
