...
`--compare` 現在會把兩邊的 `pitch` 和 `alpha` 逐段印出來對照。剩下的可能性:

- **升降舵配平**(`fcs/pitch-trim-cmd-norm`)—— 我們的 `Aircraft` 從來沒設過
- **重心位置**不同
- **俯仰力矩**係數不同(那才真的是不同的 F-16)

> 旁證:探針原本的 `LevelPolicy` 裡有一項 `+ 0.05` 的升降舵偏置。寫的人當初
> 發現需要它 —— 那可能就是這件事的影子。
