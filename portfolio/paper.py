...
#: 交易池上限。**這是一道「手按得完」的閘,不是一道策略閘。**
#:
#: 2026-09-20 執政官:「從這麼多幣種裡篩選可能八到十個流動性最好、
#: 最有交易機會的,然後系統自行去模擬開單。」
#:
#: 篩選跑完是 49 檔。指令單是人**用手在 App 上按**的:49 個倉要一個一個
#: 開,每個填本金、槓桿、止損,而且每天還要調整。7 檔已經勉強,49 不可能。
#: 而一張沒有人按得完的指令單,會讓對帳整個失去意義 —— 那時候
#: 「模擬與真實的差距」量到的是你的手速,不是策略。
#:
#: 取前 10 名是**按成交額**排的,而成交額是**結構性**條件(那是事實)。
#: ⚠️ 刻意**不**按「最近漲最多 / 最可能動」排 —— 那是預測性篩選,
#: 2026-09-08 實測過一次(按離均線排序挑),驗證段輸給隨機。
#: 要加那種排序,得走研究迴路拿證據。
MAX_UNIVERSE = 10
...
def screened_universe(min_volume: float | None = None,
    ...
    # universe.screen() 已經**按成交額由大到小**排好,所以取前 N
    # 就是取流動性最好的 N 個。這裡不重排 —— 重排一次就多一個
    # 「為什麼是這個順序」要回答。
    cap = MAX_UNIVERSE if limit is None else int(limit)
    final = ok[:cap] if cap > 0 else ok
    log.info(f"動態交易池:{len(picked)} 個過前三關、{len(ok)} 個資料齊全,"
             f"取成交額前 {len(final)} 個 -> {', '.join(final)}")
    return final
...
#
# 兩組本來就都在自己模擬交易。真正的差別只有一個:
# **指令單(要你用手按的那些)是從哪一組出的。**
#
# 原本是 MAIN(七個寫死的幣)。現在改成 SCREENED ——
# 動態篩選那一組才是「系統」,MAIN 退成對照組。
#
# ⚠️ **只准有一個開關。** 如果面板某幾格讀 PRIMARY、某幾格還讀 MAIN,
# 畫面上就會出現「指令單叫你買 X,而持倉列裡沒有 X」這種狀態 ——
# 那比完全沒切換更難查。所有對使用者說話的地方一律走 PRIMARY。
#
# ⚠️ **回測的 Calmar 1.33 是七個幣那一組的數字。** 換了交易池,
# 那個數字就不再描述現在跑的東西。前向資料要重新累積,而成績單
# 會因此說「還不知道」—— 那是對的,不是故障。
PRIMARY = SCREENED

#: 對照組:七個寫死的幣。它繼續每天記帳,因為它是 PRIMARY 的對照 ——
#: 沒有對照,「動態篩選比較好」就是一句沒有辦法否證的話。
CONTROL = MAIN
