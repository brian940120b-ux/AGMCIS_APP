...
def tr(x):
    ...
    for t in x:
        ...
        s+=f"<tr><td>{t.get('symbol','-')}</td><td>{t.get('signal','-')}</td><td>3x</td><td>{round((t.get('size_usdt') or 0)*3,2)} USDT</td><td>{t.get('entry_price','-')}</td><td>{t.get('exit_price','-')}</td><td>{t.get('stoploss','-')}</td><td>{t.get('takeprofit','-')}</td><td class='{pnlc}'>{pnl if pnl is not None else '-'}</td><td>{t.get('status','-')}</td><td>{t.get('close_reason') or t.get('opened_at','-')}</td></tr>"
    return s
...
<h2>目前持倉</h2><table><tr><th>幣種</th><th>方向</th><th>槓桿</th><th>倉位價值</th><th>進場</th><th>出場</th><th>停損</th><th>停利</th><th>盈虧</th><th>狀態</th><th>時間/原因</th></tr>{tr(o)}</table>
<h2>最近平倉</h2><table><tr><th>幣種</th><th>方向</th><th>槓桿</th><th>倉位價值</th><th>進場</th><th>出場</th><th>停損</th><th>停利</th><th>盈虧</th><th>狀態</th><th>時間/原因</th></tr>{tr(c[-10:])}</table>
