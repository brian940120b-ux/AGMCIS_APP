...
5. **`position_monitor` 用輪詢的單一價格而非 high/low**,會讓模擬勝率偏高。
6. **維持保證金分層沒有處理。** 大倉位的強平價會被低估。
7. **Survivorship bias 還在。** 需要含已下市標的的歷史資料。
8. **設定變更沒有稽核。**
9. **`rate_limit_calls` 沒有自動清理排程。**
10. **舊的 Dashboard Lite 還是一個巨大的 f-string。**
