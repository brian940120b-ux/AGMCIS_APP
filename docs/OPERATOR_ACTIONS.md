...
⚠️ **順序有意義。** 這一步會**拒絕執行**,直到上面那一步的合約規格
快照存在而且沒過期。回測要扣手續費、算資金費率、判斷強平,那些數字
沒校準的時候是保守猜測值 —— 用猜測費率跑出來的 PASS 不算數
(第二節:不得忽略費用,也不得在沒驗證的情況下宣稱結果)。

真的要在未校準的狀態下跑,加 `--allow-uncalibrated`。這時候報告 JSON
裡會多一個 `input_quality` 欄位,`trustworthy: false`。那個章拿不掉,
所以幾週之後讀到那份報告的人會知道它是在什麼條件下跑出來的。
...
同樣會在規格沒校準時拒絕執行,同樣有 `--allow-uncalibrated` 與同一個章。
