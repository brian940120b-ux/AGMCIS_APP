...
def main(argv=None) -> int:
    ...
    print(f"\n  {'停損':>6}{'掃掉':>8}{'占比':>9}{'其中賺的':>10}"
          f"{'放棄的獲利':>12}{'擋掉的虧損':>12}{'淨':>10}")
    for level in (3, 5, 8, 10, 12, 15, 20, 25, 30):
        hit = [(m, r) for m, r, _ in all_mae if m >= level]
        if not hit:
            print(f"  {level:>5}%{0:>8}{0.0:>8.1f}%{'—':>10}"
                  f"{'—':>12}{'—':>12}{'—':>10}")
            continue
        winners = [r for _, r in hit if r > 0]
        forgone = sum(winners)
        # 被停損擋掉的虧損:那一段最後虧了 -r,而停損把它封在 -level。
        # 只有在「最後虧得比停損深」的時候才有得擋。
        saved = sum(max(0.0, -r - level) for _, r in hit)
        net = saved - forgone
        print(f"  {level:>5}%{len(hit):>8}"
              f"{len(hit) / len(all_mae) * 100:>8.1f}%"
              f"{len(winners):>10}{forgone:>11.1f}%{saved:>11.1f}%"
              f"{net:>+9.1f}%")
  ...
  · 「放棄的獲利」是代價:被掃出去的部位裡,有幾段最後其實是賺的。
  · 「擋掉的虧損」是好處:被掃出去而且最後虧得比停損更深的那些,
    差額就是停損省下來的。
  · 「淨」= 擋掉的 − 放棄的。**但不要把它當成績效**:被停損掃出去
    之後,均線多半還在上面,策略會再進場一次 —— 這張表沒有模擬
    那次重進場,所以它**高估了停損的好處**。它的用途是看兩邊的
    量級誰大,不是算報酬。
