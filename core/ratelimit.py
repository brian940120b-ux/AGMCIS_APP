...
═══ 速率的數字從哪裡來(2026-09-13 查證)═══
先前這裡寫的是「我自己設的地板,沒有查過文件」。已經查了:

  · ccxt 的 bingx 實作用 `rateLimit: 100`(毫秒)—— 即 **10 rps**,
    而所有公開行情端點(klines / ticker / contracts / depth /
    fundingRate / premiumIndex)的 cost 都是 1。
    ccxt 是被最多人用的參考實作,它的節流值是有人踩過雷調出來的。
  · BingX 公告:帳戶類介面 IP 限制 **2,000 次 / 10 秒**(2025-09-10 起),
    下單端點 /openApi/swap/v2/trade/order 由 5 rps 提高到 **10 rps**。
  · ccxt 把 HTTP **429** 對到 DDoSProtection、**418** 對到 PermissionDenied
    —— 與這裡的處理方式一致。

**本系統仍然只用 5 rps**,是 ccxt 參考值的一半:

  · 這套系統實際需要的量極小 —— 每日記帳 7 個幣、巡檢十分鐘一次。
    掃池子那幾百個請求慢一倍也只是幾十秒的事。
  · 限流器要擋的不是正常運作,是**重試風暴與掃描失控**。
    設在需求的十倍還是需求的百倍,對那件事沒有差別。
  · 公開行情端點的**逐組**限制 BingX 沒有在公告裡逐條列出。
    在拿不到逐條數字的情況下,取參考實作的一半是可辯護的;
    取一個「聽起來很快」的數字不是。

接私有端點時,下單要獨立一個桶:文件寫 10 rps,那個桶設 5 rps
(同樣取一半),而且**絕不與行情共用** —— 行情把桶抽乾導致平倉單
送不出去,是這裡最不能接受的失敗。
...
# ── 我方自訂的地板 ──────────────────────────────────────
#
# 2026-09-13 **被交易所打臉一次,已下修。**
#
# 原本 5 rps / 突發 10。對帳的逐幣持倉查詢在兩秒內打了 8 個請求
# (餘額 1 + 七個幣),BingX 回 429。
#
# 學到的兩件事:
#   一、**突發才是問題,不是平均速率。** 桶允許 10 個瞬間發完,
#       而「同一個端點連續十發」正是觸發交易所每端點限制的病態模式。
#       平均下來明明很慢,但那一瞬間不是。
#   二、ccxt 的 10 rps 是**全域**參考值,而交易所還有**逐端點**的限制,
#       那個沒有公開。一個全域桶模型化不了它 —— 所以要更保守。
#
# 這套系統一天需要的請求量以百計,不是以萬計。2 rps 綽綽有餘,
# 掃全交易所那種幾百個請求的工作慢個兩分鐘也無所謂。
RATE_PER_S = 2.0
BURST = 3.0

# 下單端點的文件值(BingX 2025-10-16 公告:5 -> 10 rps)。
# **還沒有用到** —— 私有端點尚未實作。放在這裡是為了接的時候
# 不必再查一次,而且要記得給它一個獨立的桶。
DOCUMENTED_ORDER_RATE_PER_S = 10.0
