...
@pytest.mark.parametrize("gone", [
    "權益曲線", "相關性集中度</h2>", "策略監控", "今日訂單",
    # 2026-09-19:原本擋的是「事件日曆」四個字。但那四個字現在合法地
    # 出現在「還沒關掉的洞」裡 —— 反查發現它只是記錄、不是閘門,
    # 而那件事**必須寫在畫面上**。擋的應該是那張卡,不是那個詞
    # (跟「決策變數」同一個道理)。
    "<h2>事件日曆</h2>",
...
def test_the_things_the_consul_asked_to_remove_are_gone(gone):
    ...
    ⚠️ 砍的是**顯示**,不是風控 —— 相關性仍然是部位大小的閘門,
    只是不再各佔一張卡(2026-09-19 更正:**事件日曆不是閘門**,
    它只被記進帳本那一行;原本這裡跟著面板一起把「有記錄」講成
    「有閘門」)。相關性搬進了指令單裡
...
def test_the_page_has_no_card_that_is_not_the_current_system():
    ...
    expected = {
        "指令單 —— 要你自己按的",
        # 2026-09-18 執政官:「數字對不上,我只要一個,就是系統自行
        # 模擬的資訊。」原本是「模擬持倉」+「成績單」兩張,而它們
        # 真的對不上:一張讀每日記帳的檔案(00:30 的收盤價),一張用
        # 即時價。合成一張,所有「現在」的數字出自同一組價格。
        "模擬帳戶 —— 系統自己在跑的",
        "交易所帳戶 —— U 本位標準合約",
        "幣種 —— 能不能做,以及現在該不該做",
        "還沒關掉的洞",
